...
describe("admin contact lifecycle PATCH", () => {
  beforeEach(() => {
    ...
    mocks.updateContactLifecycle.mockResolvedValue(updatedContact);
  });

  it("rejects requests that bypass Basic Auth", async () => {
    const response = await patch({ leadStatus: "booked" }, false);
  ...
  it("requires a reason when a lead is marked lost", async () => {
    const response = await patch({ leadStatus: "lost", lostReason: "" });
  ...
  it("returns not found for a missing lead", async () => {
    mocks.updateContactLifecycle.mockResolvedValue(undefined);

    const response = await patch({ leadStatus: "contacted" });
