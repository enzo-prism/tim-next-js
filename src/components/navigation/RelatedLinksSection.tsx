      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
        {links.map((link) => (
          <Link key={link.href} href={link.href} className="block h-full">
            <div className="h-full rounded-xl border border-gray-200 bg-white p-6 shadow-sm hover:shadow-sm transition-shadow duration-200">
              <div className="flex items-start justify-between gap-3">
                ...
                <MinimalGlyph name="arrow-right" className="h-5 w-5 text-primary flex-shrink-0 mt-1" />
              </div>
            </div>
          </Link>
        ))}
